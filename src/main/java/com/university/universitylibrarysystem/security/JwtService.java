...
public class JwtService {
    ...
    /**
     * Extract any claim using a lambda function
     */
    ...
    /**
     * Generate token for UserDetails (existing librarians/admins)
     */
    public String generateToken(UserDetails userDetails) {
        // include role claim for librarians/managers
    ...
    claims.put("role", "ROLE_STUDENT"); // ✅ FIX
    claims.put("studentId", student.getStudentId());

    return Jwts.builder()
            .setClaims(claims)
            .setSubject(student.getStudentId())
            .setIssuedAt(new Date())
            .setExpiration(new Date(System.currentTimeMillis() + jwtExpirationMs))
            .signWith(getSignInKey(), SignatureAlgorithm.HS256)
            .compact();

...
export default function LibrarianDashboard() {
  ...
  const [editForm, setEditForm] = useState({
    ...
    department: '', major: '', minorSubject: '', yearLevel: '',
    phoneNumber: '', universityCardId: '',
  });
  ...
  async function handleEditSubmit() {
    ...
    setEditLoading(true);
    try {
      await studentService.updateStudent(editingStudentDbId, {
        ...editForm,
        yearLevel: editForm.yearLevel ? parseInt(editForm.yearLevel) : null,
      });
    ...
    <div style={{ padding: 20, fontFamily: "'Segoe UI', sans-serif", backgroundColor: '#f0f4f8', minHeight: '100vh' }}>
      ...
      {editModalOpen && (
        <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, backgroundColor:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ backgroundColor:COLORS.white, borderRadius:10, padding:'28px 32px', width:'100%', maxWidth:500, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 8px 32px rgba(0,0,0,0.18)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              {[
                ...
                { label:'Minor Subject', key:'minorSubject', type:'text' },
                { label:'Year Level (1-4)', key:'yearLevel', type:'number' },
                { label:'Phone Number', key:'phoneNumber', type:'text' },
                { label:'University Card ID', key:'universityCardId', type:'text' },
              ].map(({ label, key, type }) => (
                <div key={key}>
                  <label style={{ fontSize:'0.78em', color:COLORS.gray, display:'block', marginBottom:4, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.04em' }}>{label}</label>
                  <input
                    type={type}
                    value={editForm[key] ?? ''}

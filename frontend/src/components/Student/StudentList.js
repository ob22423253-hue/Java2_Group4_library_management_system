  ...
  return (
    <div>
      <div style={{ display:'flex', gap:10, marginBottom:14, alignItems:'center', flexWrap:'wrap' }}>
        <input type="text" placeholder="Search by name, ID, email, department..."
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', backgroundColor:COLORS.white }}>
          <thead>
            <tr style={{ backgroundColor:COLORS.primary, color:'white' }}>
              {['Full Name','Student ID','Email','Department','Actions'].map(h => (
          ...
          <tbody>
            {paginated.length === 0 ? (
              <tr><td colSpan={5} style={{ padding:30, textAlign:'center', color:COLORS.gray }}>No students match your search</td></tr>
            ) : paginated.map((s, idx) => (
